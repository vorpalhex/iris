'use strict';

//modules
const chrono = require('chrono-node');
const moment = require('moment');
const gAuth = new (require('google-auth-library'))();
const cal = (require('googleapis')).calendar('v3');

//local
const bus = require(__dirname + '/../core/bus');
const log = require(__dirname + '/../util/log')();
const csplit = require(__dirname + '/../util/commandSplit');
let conf = null;

try {
  conf = require(__dirname + '/../util/conf').commands.events;
} catch (e) {
  console.log('Unable to load events config', e);
}

process.env['GOOGLE_APPLICATION_CREDENTIALS'] = conf.secret;
const calendarId = conf.calendar;

function auth() {
  return new Promise((resolve, reject) => {
    gAuth.getApplicationDefault((err, authClient)=>{
      if(err) return reject(err);

      if (authClient.createScopedRequired && authClient.createScopedRequired()) {
        const scopes = [
          'https://www.googleapis.com/auth/calendar'
        ];
        authClient = authClient.createScoped(scopes);
      }

      return resolve(authClient);
    });
  });
}

function addEvent(name, start, end, location) {
  start = chrono.parseDate(start);
  end = chrono.parseDate(end);

  return auth()
  .then((auth) => {
    return new Promise( (resolve, reject) => {
      const resource = {
        start: {
          dateTime: start,
        },
        end: {
          dateTime: end,
        },
        summary: name,
        location
      };

      cal.events.insert({
        auth,
        calendarId,
        resource,
      }, (err, event) => {
        if (err) return reject(err);

        return resolve(event);
      });
    });
  });
}

function getEvents(maxResults=5) {
  return new Promise( (resolve, reject) => {
    auth().then((auth) => {
      cal.events.list({
        auth,
        calendarId,
        maxResults,
        timeMin: (new Date).toISOString(),
        singleEvents: true,
        orderBy: 'startTime'
      }, (err, response) => {
        if (err) return reject(err);
        return resolve((response && response.items) || []);
      });
    })
  });
}

module.exports = {
  register: ()=>{
    bus.subscribe('message.direct', (eName, message) => {
      let words = csplit(message.contents.toLowerCase());

      if(!words[0] || words[0] !== 'events') return;

      switch(words[1]) {
        //events add "my event name" "start" "end" "location"
        case 'add':

          if(words.length < 5) return message.respond('events add "my event name" "tomorrow at 9pm" "tomorrow at 10pm" "Opals" ');

          const name = words[2];
          const start = words[3];
          const end = words[4];
          const location = words[5] || null;

          addEvent(name, start, end, location)
            .then( () => message.respond('Event created!') )
            .catch( (e) => {
              log.error('Error creating event', words, e);
              return message.respond('Errrr, hm. That did not quite work. Try that again?');
            })

          break;
        case 'list':

          getEvents(5)
          .then( (events) => {
            const lines = ["Here's what I've got on the schedule:", '', ''];

            if(!events.length) return message.respond('No events coming up!');

            events.forEach( (event) => {
              lines.push(`**${event.summary}** ${event.creator.email}`);
              lines.push(` ${moment(event.start.dateTime).calendar(null, {sameElse: 'DD/MM/YYYY h:mma'})} - ${moment(event.end.dateTime).calendar(null, {sameElse: 'DD/MM/YYYY h:mma'})}`);
              lines.push(`*${event.location || 'No location provided'}*`);
              lines.push('---');
              lines.push(event.description);
              lines.push('');
              lines.push(event.htmlLink);
              lines.push('');
              lines.push('');
              lines.push('');
            });

            return message.respond(lines.join('\n'));
          })
          .catch( (e) => {
            log.error('Error getting events', e);
            return message.respond('Errrr, hm. That did not quite work. Try that again?');
          });
          break;
        default:
          return message.respond('Valid commands: add, list');
          break;
      }
    });
    return true;
  },
  help: ()=>{
    return {
      cmd: 'events',
      description: 'add new events and announce upcoming ones',
      aliases: []
    };
  }
}
