...
function listEvents(message) {
  ...
  .then( (events) => {
    ...
    events.forEach( (event) => {
      ...
      lines.push(` ${moment(event.start.dateTime).calendar(null, {sameElse: 'MM/DD/YYYY h:mma'})} - ${moment(event.end.dateTime).calendar(null, {sameElse: 'MM/DD/YYYY h:mma'})}`);
    ...
    return responder(output, {split: true});
  })
  .catch( (e) => {
