...
function savePreviousMessages(eName, message) {
  ...
  if (command && command !== 'comic') {
    previousMessages[message.channel] = _.takeRight(previousMessages.concat([{user, text}]), previousMessagesToKeep);
  }
}
...
function runCommand(eName, message) {
  ...
  let comicText = previousMessages[message.channel].splice(0, requestedLineNumber);
