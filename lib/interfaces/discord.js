...
function registerHandlers(){
  discord.on('presenceUpdate', (old, newP)=>{
    resolveUser(newP, (err, user)=>{
      ...
      if(presence.user.displayName === conf.name || presence.user.displayName === 'me') return;
  ...
  discord.on('message', (msg)=>{
    resolveUser(msg.member, (err, user)=>{
      let message = {
        ...
        mentions: [],
        channel: `discord_${msg.channel.guild}_${msg.channel.name}`
      };
      ...
      async.each(mentions, (mention, cb)=> {
        ...
        if(message.contents.indexOf('@'+conf.name) === 0){
          ...
          return bus.publish('message.direct', dm);
        }
